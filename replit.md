...
- **offset 투명화(보정 레버 ↔ 자가학습 충돌)**: `GET /learning/offsets`(`getAppliedOffsetBreakdown`, `populationData.ts`)가 auto(학습)·manual(보정 레버)·combined offset과 같은방향 누적 clamp(`OFFSET_LIMITS` political35/consumer20/policy20) 여부를 반환. 보정·검증 화면 `AppliedOffsetPanel`이 총 적용 기준선 이동 + clamp 경고 표시.
- **정직한 calibratedError(정치)**: `calibratedError = rawError*0.6` 하드코딩 제거 → 전역 Dynamo 이벤트 출력보정 모델(`globalOutputCalibrationCorrection`, applied면 shrinkage*meanBias)을 실제 적용. 모델 미적용 시 `calibratedError=rawError`(정직). 소비/정책 error는 ground-truth 부재로 여전히 illustrative.
...
- **실시간 신호 인제스트**(투자자 데모): **실제 파이프라인** — Google 뉴스 RSS fetch + LLM(gpt-5-mini) 감성·요약 분석(`signalIngest.ts`). 소스는 **'뉴스'만** 실수집(검색트렌드·SNS는 400). 수집 실패 시 **명시적 502**(가짜 fallback 금지). 수동(`POST /admin/signals`)·자동(`/admin/signals/auto`) 모두 실파이프라인. **시드 리셋(`/admin/signals/reset`)만** 결정론적 목업(`signalMock.ts` RESET_SCENARIOS)로 데모 초기상태 복원. 엔진/보정 미수정, 전역 공유(userId=0).
